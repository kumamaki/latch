...
public enum LatchCatalog {
    ...
    ///
    /// Ordered-out named windows stay. Only owns rows this helper
    /// registered. `.latchWindow` keeps its own token, so a dump cannot
    /// steal `window.main` out from under it.
